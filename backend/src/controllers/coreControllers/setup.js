...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      brickflow_app_email: email,
      brickflow_app_company_email: email,
      brickflow_app_timezone: timezone,
      brickflow_app_country: country,
      brickflow_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully BrickFlow App Setup',
  });
};
